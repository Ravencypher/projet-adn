<template>
  <div class="row g-0 d-flex flex-wrap justify-content-between">
    <div class="col-3 m-4 d-flex flex-column align-items-center flex-fill">
      <figure>
        <img src="../assets/darwin.jpg" alt="image boycott" />
      </figure>
    ...
    >
    <div class="card-label">Date d'inscription</div>
      <div class="card-field px-3 py-2 my-2">
      {{ formatDate(user?.createdAt) }}
      </div>
      <div class="card-label">Email</div>
      <div class="card-field px-3 py-2 my-2">{{ user?.email }}</div>
      <div class="card-label">Ville</div>
      <div class="card-field px-3 py-2 my-2">{{ user?.ville }}</div>
      <div class="card-label">Pays</div>
      <div class="card-field px-3 py-2 my-2">{{ user?.pays }}</div>
    </div>
  </div>
</template>
